...
def _rate_limit_sleep():\
    # Gentle pacing so you never hit PubMed rate limits.\
...
\
        # Abstract (can be split into multiple <AbstractText> parts)\
...
if __name__ == "__main__":\
    # Quick smoke test\
    demo_query = 'omaveloxolone AND (Friedreich* OR "Friedreich Ataxia") AND ferroptosis'\
    out = search_pubmed(demo_query, max_results=5, mindate="2018")\
    print(f"Found \{out['count']\} results")\
    if out["results"]:\
        first = out["results"][0]\
        print(first["title"])\
        print(first["url"])\
